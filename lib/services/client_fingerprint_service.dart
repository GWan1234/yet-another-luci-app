...
class ClientFingerprintService {
  ...
  Future<void> initialize() async {
    ...
    try {
      jsonContent = await rootBundle.loadString('assets/fingerprints/advanced.json');
    } catch (_) {
    ...
    if (jsonContent == null || jsonContent.isEmpty) {
      try {
        jsonContent = await rootBundle.loadString('assets/fingerprints/default.json');
      } catch (e) {
        if (kDebugMode) {
          print('ClientFingerprintService: Failed to load default fingerprints: $e');
        }
      }
    }
      ...
      for (final item in hostList) {
        if (item is Map<String, dynamic>) {
          ...
          if (rawPat.isNotEmpty) {
            try {
              _hostnameRules.add(HostnameRule(
                rawPattern: rawPat,
                match: FingerprintMatch.fromJson(item),
              ));
            } catch (_) {}
