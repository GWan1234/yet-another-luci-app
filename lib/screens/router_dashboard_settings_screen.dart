...
class _RouterDashboardSettingsScreenState
  ...
  static const Map<String, ({String name, IconData icon, String desc})> _cardMeta = {
    'quick_actions': (
      name: 'Quick Actions Bar',
      icon: Icons.flash_on,
      desc: 'Shortcuts for reboot, flush DNS cache, guest Wi-Fi, VPNs & refresh',
    ),
    'device_info': (
    ...
    'realtime_traffic': (
      ...
      desc: 'Live upload/download speed graph and context-aware interface throughput',
    ),
    'system_vitals': (
      name: 'System Vitals',
    ...
    'wireless_networks': (
      ...
      icon: Icons.wifi,
      desc: 'Wi-Fi channels, signal quality, encryption, and associated stations',
    ),
    'network_interfaces': (
      name: 'Network Interfaces',
      icon: Icons.lan,
      desc: 'WAN/LAN IP addresses, RX/TX transfer stats, subnets & bridge devices',
    ),
    'system_modules': (
      name: 'System Modules & Storage',
      icon: Icons.storage,
      desc: 'Storage overview, filesystem usage, and dynamic service status cards',
    ),
  };
  ...
  Widget _buildCardOrderSection() {
    ...
    return _buildSection(
      ...
      subtitle: 'Drag handle to reorder dashboard cards or toggle switch to show/hide sections',
                ...
                trailing: Switch.adaptive(
                  ...
                  onChanged: (val) {
                    setState(() {
                      switch (cardId) {
                        case 'quick_actions':
                          _preferences = _preferences.copyWith(showQuickActions: val);
                          break;
                        case 'device_info':
                          _preferences = _preferences.copyWith(showDeviceInfo: val);
                          break;
                        case 'realtime_traffic':
                          _preferences = _preferences.copyWith(showRealtimeTraffic: val);
                          break;
                        case 'system_vitals':
                          _preferences = _preferences.copyWith(showSystemVitals: val);
                          break;
                        case 'connected_clients':
                          _preferences = _preferences.copyWith(showConnectedClients: val);
                          break;
                        case 'wireless_networks':
                          _preferences = _preferences.copyWith(showWirelessNetworks: val);
                          break;
                        case 'network_interfaces':
                          _preferences = _preferences.copyWith(showNetworkInterfaces: val);
                          break;
                        case 'system_modules':
                          _preferences = _preferences.copyWith(showSystemModules: val);
                          break;
                      }
                    });
                    _onPreferenceChanged();
  ...
  Widget _buildQuickActionsSection() {
    const actions = [
      (id: 'reboot', title: 'Reboot Router', icon: Icons.restart_alt, desc: 'Quick restart trigger with confirmation'),
      (id: 'flush_dns', title: 'Flush DNS Cache', icon: Icons.cleaning_services, desc: 'Restart dnsmasq to clear DNS resolver cache'),
      (id: 'guest_wifi', title: 'Guest Wi-Fi Shortcut', icon: Icons.wifi_tethering, desc: 'Jump to guest Wi-Fi management'),
      (id: 'vpn', title: 'VPN Management Shortcut', icon: Icons.vpn_key, desc: 'Jump to VPN configuration & status'),
      (id: 'refresh', title: 'Refresh Data Button', icon: Icons.refresh, desc: 'Force reload dashboard metrics'),
    ];

    return _buildSection(
      title: 'Quick Action Shortcuts',
      subtitle: 'Select shortcut action buttons (Reboot, Flush DNS, Guest Wi-Fi, VPN, Refresh) for your dashboard',
      icon: Icons.flash_on,
      children: [
        ...actions.map((act) {
          ...
          return Padding(
            ...
            child: CheckboxListTile(
              title: Text(act.title, style: LuciTextStyles.detailValue(context)),
              subtitle: Text(act.desc, style: LuciTextStyles.cardSubtitle(context)),
              secondary: Icon(
                act.icon,
                size: 20,
                color: isEnabled
              ...
              onChanged: (val) {
                setState(() {
                  final newSet = Set<String>.from(_preferences.enabledQuickActions);
                  if (val ?? false) {
                    newSet.add(act.id);
                  } else {
                    newSet.remove(act.id);
                  }
                  _preferences = _preferences.copyWith(enabledQuickActions: newSet);
                });
                _onPreferenceChanged();
              },
  ...
  Widget _buildSystemVitalsOptionsSection() {
    return _buildSection(
      ...
      subtitle: 'Choose hardware performance metrics (CPU, RAM, Load Average, Uptime) to display',
      icon: Icons.monitor_heart,
      children: [
  ...
  Widget _buildTrafficAndUnitsSection() {
    ...
    return _buildSection(
      ...
      subtitle: 'Select throughput speed units (Mbps vs MB/s) and interface monitoring scope',
      icon: Icons.speed,
      children: [
        Padding(
        ...
        Container(
          ...
          child: SwitchListTile.adaptive(
            ...
            subtitle: const Text('Aggregate overall throughput across all interfaces'),
    ...
    return _buildSection(
      ...
      children: [
        SwitchListTile.adaptive(
          ...
          subtitle: const Text('Hide public IP address by default for privacy & screenshots'),
        ...
        SwitchListTile.adaptive(
          ...
          subtitle: const Text('Display interfaces even if offline or unassigned'),
