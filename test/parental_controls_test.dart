...
void main() {
  ...
  group('ParentalControlsController & Sync Hardening Tests', () {
    ...
    test('Pause expiry on checkPausesAndSchedules automatically resumes expired profiles', () async {
      final pastExpiry = DateTime.now().toUtc().subtract(const Duration(minutes: 5));
      final profile = ParentalProfile(
    ...
    test('Store persistence round-trip preserves daily minutes and activity log', () {
      final profile = ParentalProfile(
        id: 'test_rt',
    ...
    test('Clear activity log via controller clears log and persists store', () async {
      final appState = AppState.instance;
      final profile = ParentalProfile(
        id: 'test_clear_log',
        name: 'Log Test',
    ...
    test('loadStore and persistStore return typed ParentalActionResult', () async {
      final appState = AppState.instance;
