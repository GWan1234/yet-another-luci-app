...
void main() {
  group('EntitlementTier Model & Router Limit Unit Tests', () {
    test('Free tier allows max 1 router and displays ads', () {
      const state = EntitlementState(tier: EntitlementTier.free);
      expect(state.routerLimit, equals(1));
      expect(state.isAdFree, isFalse);

      expect(state.canAddRouter(0), isTrue); // Can add 1st router
      expect(state.canAddRouter(1), isFalse); // Blocked from adding 2nd router
      expect(state.canAddRouter(5), isFalse);
    });

    test('Plus tier allows max 3 routers and is ad-free', () {
      const state = EntitlementState(tier: EntitlementTier.plus);
      expect(state.routerLimit, equals(3));
      expect(state.isAdFree, isTrue);

      expect(state.canAddRouter(0), isTrue);
      expect(state.canAddRouter(1), isTrue);
      expect(state.canAddRouter(2), isTrue); // Can add 3rd router
      expect(state.canAddRouter(3), isFalse); // Blocked from adding 4th router
    });
  ...
  group('EntitlementState Transitions', () {
    test('State copyWith correctly updates entitlement tier and retains state flags', () {
      ...
      expect(plusState.routerLimit, equals(3));
